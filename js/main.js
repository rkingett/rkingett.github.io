(() => {
  ...
  window.addEventListener("DOMContentLoaded", () => {
    const nav = (() => {
      const nav2 = document.querySelector(".main-navigation");
      const navOpen = document.querySelector(".mobile-nav-open");
      const navClose = document.querySelector(".mobile-nav-close");
      navOpen.addEventListener("click", () => nav2.classList.add("active"));
      navClose.addEventListener("click", () => nav2.classList.remove("active"));
    })();
    const search = (() => {
      const searchResult = (result) => `<div class="search-result"><a href="${result.url}">${result.name}</a></div>`;
      const search2 = document.querySelector("#search-form");
      ...
      search2.addEventListener("submit", (e) => {
        ...
        const results = Array.from(pages)?.filter(
